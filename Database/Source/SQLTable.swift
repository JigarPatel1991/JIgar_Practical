...
class SQLTable:NSObject {
    ...
    func selectRowsWhere<T:LearnVern>(strwhere:String="") -> [T] {
      
        var res = [T]()
        self.data = values()
        let db = SQLiteDB.sharedInstance()
        let sql = "SELECT * FROM \(table) WHERE date = '\(strwhere)'"
    ...
    func selectAllRows<T:LearnVern>(strwhere:String="") -> [T] {
